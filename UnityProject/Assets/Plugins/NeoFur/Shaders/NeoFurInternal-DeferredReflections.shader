Shader "Hidden/NeoFurInternal-DeferredReflections" {
Properties {
	_SrcBlend ("", Float) = 1
	_DstBlend ("", Float) = 1
}
...
half4 frag (unity_v2f_deferred i) : SV_Target
{
	...
	if (shadingID == 0)
	{
		//Sometimes this looks better, but sometimes it causes weird artifacts.
		/*
		float3 unpackedNormal = SphericalToNormal(gbuffer1.yz * 2 - 1);
		gbuffer2.rgb = unpackedNormal*0.5+0.5;
		*/

		//Specular gbuffer does not contain specular information.
		gbuffer1.rgb = 0;
	}
